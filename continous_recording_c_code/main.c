...
int main() {
	...
	/* Stay in this loop until the FSM writes the desired amount of samples */
	while (alt_read_word(h2p_lw_audio_addr + 2) == 0x0){	// If 1 then the FSM finished
		if (alt_read_word(h2p_lw_audio_addr + 3) == 0x1){	// Halfway signal from the FSM
			grabData(ddr3_5, f5_l, f5_r, 0x0, HALF_BUF);	// Grab the data from the lower half of memory
			/* Acknowledge the Halfway signal from Hardware*/
			while(alt_read_word(h2p_lw_audio_addr + 3) == 0x1){alt_write_word(h2p_lw_audio_addr + 7, 0x0);}
		}
		if(alt_read_word(h2p_lw_audio_addr + 4) == 0x1){	 	// End signal from the FSM (FSM now wraps around to the beginning)
			grabData(ddr3_5, f5_l, f5_r, HALF_BUF, BUF_SIZE);	// Grab the upper half of memory
			/* Ackknowledge the End signal from Hardware*/
			while(alt_read_word(h2p_lw_audio_addr + 4) == 0x1){alt_write_word(h2p_lw_audio_addr + 7, 0x1);}
		}
	}
	alt_write_word(h2p_lw_audio_addr, 0x0); // Reset the FSM
	/* Collect the left over data that resides in memory */
